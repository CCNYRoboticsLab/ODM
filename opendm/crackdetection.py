...
class CrackDetector:  # Renamed the class
    ...
    def create_overlay(self, original_image, segm_mask):
        ...
        # Print unique values in resized_segm_mask
        unique_values = np.unique(resized_segm_mask)
        print(f"Unique values in resized_segm_mask: {unique_values}")
        ...
        # Create a green color mask for crack
        crack_mask = np.zeros_like(original_image)
        crack_mask[resized_segm_mask == 127] = [0, 255, 0]  # Set green color for crack

        # Directly modify the original image based on the mask
        original_image[resized_segm_mask == 127] = crack_mask[resized_segm_mask == 127]
